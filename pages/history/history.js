...
Page({
  ...
  loadHistory: function () {
    ...
    db.collection('histories').where({
      userId: userId,
    }).orderBy('viewTime', 'desc').get().then(res => {
      ...
      if (scenicIds.length > 0) {
        promises.push(db.collection('scenics').where({
          _id: db.command.in(scenicIds)
        }).get().then(res => {
          return res.data.map(item => ({ ...item, type: 'scenic' }));
        }));
      } else {
      ...
      if (foodIds.length > 0) {
        promises.push(db.collection('foods').where({
          _id: db.command.in(foodIds)
        }).get().then(res => {
          return res.data.map(item => ({ ...item, type: 'food' }));
        }));
      } else {
        promises.push(Promise.resolve([]));
      }
